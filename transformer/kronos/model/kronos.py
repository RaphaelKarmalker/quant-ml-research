...
class Kronos(nn.Module, PyTorchModelHubMixin):
    ...
    Args:
        ...
        time_feature_list (list, optional): List of time features to use. Defaults to ['minute', 'hour', 'weekday', 'day', 'month'].
    """

    def __init__(self, s1_bits, s2_bits, n_layers, d_model, n_heads, ff_dim, ffn_dropout_p, attn_dropout_p, resid_dropout_p, token_dropout_p, learn_te, time_feature_list=None):
        super().__init__()
        self.s1_bits = s1_bits
        self.s2_bits = s2_bits
        ...
        self.time_feature_list = list(time_feature_list) if time_feature_list is not None else ['minute', 'hour', 'weekday', 'day', 'month']

        self.s1_vocab_size = 2 ** self.s1_bits
        self.token_drop = nn.Dropout(self.token_dropout_p)
        self.embedding = HierarchicalEmbedding(self.s1_bits, self.s2_bits, self.d_model)
        self.time_emb = TemporalEmbedding(self.d_model, self.learn_te, self.time_feature_list)
...
def calc_time_stamps(x_timestamp, extra_df=None, time_feature_list=None, base_ts_since_listing=None):
    """
    Build time features from timestamps. Accepts pd.Series, pd.DatetimeIndex, or array-like.

    Optional:
      - extra_df: DataFrame providing additional time-feature columns (e.g., 'ts_since_listing') for x window.
      - time_feature_list: list of desired time features and order.
      - base_ts_since_listing: if provided, generate 'ts_since_listing' for y window by incrementing from this base.
    """
    # Coerce to pandas Series[datetime64[ns]] to ensure `.dt` is available
    if isinstance(x_timestamp, pd.Series):
        ts = pd.to_datetime(x_timestamp, errors="coerce")
    elif isinstance(x_timestamp, pd.DatetimeIndex):
        ts = pd.Series(x_timestamp)
    else:
        ts = pd.Series(pd.to_datetime(x_timestamp, errors="coerce"))

    if ts.isnull().any():
        raise ValueError("calc_time_stamps received non-parsable timestamps.")

    time_df = pd.DataFrame()
    # Always construct standard categorical time components (legacy)
    ...
    # Inject optional numeric feature 'ts_since_listing' if requested
    if time_feature_list is not None and ('ts_since_listing' in time_feature_list):
        if (extra_df is not None) and ('ts_since_listing' in extra_df.columns):
            # Align by last len(x_timestamp) rows
            vals = np.asarray(extra_df['ts_since_listing'][-len(ts):], dtype=np.int64)
            if len(vals) != len(ts):
                raise ValueError("Length mismatch for ts_since_listing between extra_df and timestamps.")
            time_df['ts_since_listing'] = vals
        elif base_ts_since_listing is not None:
            n = len(ts)
            base = int(base_ts_since_listing)
            time_df['ts_since_listing'] = np.arange(1, n + 1, dtype=np.int64) + base
        else:
            # Fallback to zeros to avoid crash (still allows inference)
            time_df['ts_since_listing'] = np.zeros(len(ts), dtype=np.int64)

    # If a list is provided, keep only requested and in the specified order when available
    if time_feature_list is not None:
        cols = [c for c in time_feature_list if c in time_df.columns]
...
class KronosPredictor:
    ...
    def __init__(self, model, tokenizer, device="cuda:0", max_context=512, clip=5, feature_cols=None, time_cols=None):
        self.tokenizer = tokenizer
        self.model = model
        self.max_context = max_context
        self.clip = clip
        # use generic feature columns; fallback keeps old default to avoid breaking callers
        self.feature_cols = feature_cols 
        # allow caller/config to define time feature columns and order
    ...
    def predict(self, df, x_timestamp, y_timestamp, pred_len, T=1.0, top_k=0, top_p=0.9, sample_count=1, verbose=True):
        ...
        # select and order time features according to self.time_cols
        if self.time_cols:
    ...
    def predict_batch(self, df_list, x_timestamp_list, y_timestamp_list, pred_len, T=1.0, top_k=0, top_p=0.9, sample_count=1, verbose=True):
        ...
        for i in range(num_series):
            ...
            pred_df = pd.DataFrame(preds_i, columns=self.feature_cols, index=y_timestamp_list[i])
