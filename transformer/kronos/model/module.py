...
class TemporalEmbedding(nn.Module):
    def __init__(self, d_model, learn_pe, time_feature_list=None):
        super(TemporalEmbedding, self).__init__()
        # Allow None -> legacy full list
        self.time_feature_list = list(time_feature_list) if time_feature_list is not None else ['minute', 'hour', 'weekday', 'day', 'month']
        # Size map for categorical (index-based) features
        size_map = {
            'minute': 60,
            'hour': 24,
            'weekday': 7,
        ...
        # Categorical features -> Embedding; Numeric features -> Linear(1 -> d_model)
        Embed = FixedEmbedding if not learn_pe else nn.Embedding
        self.embeddings = nn.ModuleDict()
        self.numeric_proj = nn.ModuleDict()
        self.is_numeric = {}

        for feat in self.time_feature_list:
            if feat in size_map:
                self.embeddings[feat] = Embed(size_map[feat], d_model)
                self.is_numeric[feat] = False
            else:
                # Treat unknown features (e.g., 'ts_since_listing') as numeric scalars
                self.numeric_proj[feat] = nn.Linear(1, d_model, bias=False)
                self.is_numeric[feat] = True

    def forward(self, x):
        # x: [B, T, len(time_feature_list)] in the SAME order as time_feature_list
        if x.size(-1) != len(self.time_feature_list):
            raise ValueError(f"TemporalEmbedding expected {len(self.time_feature_list)} features, got {x.size(-1)}.")
        emb_sum = 0
        for idx, feat in enumerate(self.time_feature_list):
            if not self.is_numeric.get(feat, False):
                emb_sum = emb_sum + self.embeddings[feat](x[:, :, idx].long())
            else:
                # Numeric scalar feature (e.g., 'ts_since_listing')
                emb_sum = emb_sum + self.numeric_proj[feat](x[:, :, idx:idx+1].float())
        return emb_sum
