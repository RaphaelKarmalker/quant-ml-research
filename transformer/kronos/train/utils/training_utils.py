...
def setup_ddp():
    ...
    # Windows kann kein NCCL
    backend = "nccl"
    if os.name == "nt":  # Windows
        backend = "gloo"

    # Wenn kein torchrun gestartet wurde → Single-GPU
    if "RANK" not in os.environ or "WORLD_SIZE" not in os.environ:
        print("[INFO] No torchrun detected → running in single-GPU mode")
        rank, world_size, local_rank = 0, 1, 0
        if torch.cuda.is_available():
            torch.cuda.set_device(local_rank)
        return rank, world_size, local_rank

    # Multi-GPU / torchrun Modus
...
def safe_barrier():
    """Only calls dist.barrier() if a process group exists (DDP mode)."""
